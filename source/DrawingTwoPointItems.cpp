...
void DrawingLineItem::setLine(const QLineF& line)
{
	...
	if (points.size() >= 2)
	{
		points.first()->setPos(line.p1());
		points.last()->setPos(line.p2());
	}

	if (points.size() >= 2) points[1]->setPos((line.p1() + line.p2()) / 2);
}
...
{
	return (DrawingItem::canRemovePoint(point) && points().size() > 2 && point != startPoint() && point != endPoint());
}
