...
QRectF DrawingRectResizeItem::boundingRect() const
{
	...
	if (isValid())
	{
		...
		rect = DrawingRectResizeItem::rect().normalized();
...
void DrawingRectResizeItem::resizeItem(DrawingItemPoint* itemPoint, const QPointF& scenePos)
{
	...
	if (points.size() >= 8)
	{
		QRectF rect = DrawingRectResizeItem::rect();
...
QPainterPath DrawingRectItem::shape() const
{
	...
	if (isValid())
	{
		...
		// Add rect
		drawPath.addRoundedRect(rect(), cornerRadiusX(), cornerRadiusY());
...
void DrawingRectItem::paint(QPainter* painter)
{
	if (isValid())
	{
		...
		painter->setPen(pen());
		painter->drawRoundedRect(rect(), cornerRadiusX(), cornerRadiusY());
...
QRectF DrawingPolygonItem::boundingRect() const
{
	...
	{
		qreal penWidth = pen().widthF();
		rect = polygon().boundingRect().normalized();
		rect.adjust(-penWidth/2, -penWidth/2, penWidth/2, penWidth/2);
	}
	return rect;
}
...
{
	return DrawingItem::canInsertPoint(point);
}

bool DrawingPolygonItem::canRemovePoint(DrawingItemPoint* point) const
{
	return (DrawingItem::canRemovePoint(point) && points().size() > 3);
}
...
int DrawingPolygonItem::insertPointIndex(DrawingItemPoint* point) const
{
	...
	{
		QList<DrawingItemPoint*> points = DrawingPolygonItem::points();
		qreal minimumDistance = distanceFromPointToLineSegment(point->pos(),
		...
		{
			distance = distanceFromPointToLineSegment(point->pos(),
